...
nhefs_with_cat <- nhefs_complete %>%
  mutate(
    ...
    alcoholfreq_cat = factor(alcoholfreq_cat, 
                             levels = c("none", "lt_12_per_year", "1_4_per_month", 
                                        "2_3_per_week", "daily", "unknown"))
  )
...
cat_ps_model <- multinom(
  alcoholfreq_cat ~ sex + race + age + I(age^2) + education + 
...
cat_ps <- as.data.frame(predict(cat_ps_model, type = "probs", newdata = nhefs_for_model))
...
nhefs_with_cat_weights <- nhefs_for_model %>%
  mutate(
    ...
    w_cat_att_none = as.numeric(wt_att(cat_ps, alcoholfreq_cat, focal = "none")),
    w_cat_att_lt12 = as.numeric(wt_att(cat_ps, alcoholfreq_cat, focal = "lt_12_per_year")),
    w_cat_att_1_4mo = as.numeric(wt_att(cat_ps, alcoholfreq_cat, focal = "1_4_per_month")),
    w_cat_att_2_3wk = as.numeric(wt_att(cat_ps, alcoholfreq_cat, focal = "2_3_per_week")),
    w_cat_att_daily = as.numeric(wt_att(cat_ps, alcoholfreq_cat, focal = "daily")),
    w_cat_atu_none = as.numeric(wt_atu(cat_ps, alcoholfreq_cat, focal = "none")),
