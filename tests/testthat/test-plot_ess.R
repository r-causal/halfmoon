...
test_that("plot_ess works with groups", {
  ...
  col_layer <- p$layers[[which(sapply(
    p$layers,
    function(l) inherits(l$geom, "GeomCol")
  ))]]
...
test_that("plot_ess labels work correctly", {
  ...
  text_layer_with <- sapply(
    p_with_labels$layers,
    function(l) inherits(l$geom, "GeomText")
  )
  text_layer_without <- sapply(
    p_without_labels$layers,
    function(l) inherits(l$geom, "GeomText")
  )
...
test_that("plot_ess customization works", {
  ...
  hline_layer <- p$layers[[which(sapply(
    p$layers,
